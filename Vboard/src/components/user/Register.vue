            ...
            <v-card-text>
              <v-container>
                <v-row>
                  ...
                  <v-col cols="6">
                    <v-btn
                      :loading="checkUidLoading"
                      color="success"
                      @click="btnCheckUid"
                      >중복확인</v-btn
                    >
                    <v-chip
                      v-if="user.checkUidNo"
                      class="ma-2"
                      color="red"
                      text-color="white"
                    >
...
const checkUidLoading = ref(false);
...
const btnCheckUid = () => {
  if (!user.uid.trim) {
    ...
    axios
      .get("http://localhost:8080/Vboard/user/countUid", {
        params: { uid: user.uid },
      })
      .then((response) => {
        setTimeout(() => {
          checkUidLoading.value = false;
          if (response.data > 0) {
            user.checkUidNo = true;
            user.checkUidOk = false;
          } else {
            user.checkUidOk = true;
            user.checkUidNo = false;
          }
        }, 500);
      })
      .catch((error) => {
...
const btnRegister = function () {
  axios
    .post("http://localhost:8080/Vboard/user/register", user)
    .then((response) => {
      router.push("/list");
    })
    .catch((error) => {
